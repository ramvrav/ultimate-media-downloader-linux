...
class debug {
public:
  debug(const QString &e = QString()) : m_switch(e) {}
  template <typename T> debug &operator<<(const T &e) { return _print(e); }
  debug &operator<<(const QString &e);
  debug &operator<<(const QByteArray &e);
  debug &operator<<(const QStringList &e);

private:
  template <typename T> debug &_print(const T &e) {
    if (!m_switch.isEmpty()) {
...
public:
  ...
  static QString mediaDownloaderCWD() { return "{MediaDownloaderCWD}"; }
  static QString defaultPath() { return "${default}"; }
  static QString backendPath() { return "${BackendPath}"; }
  static QString commandName() { return "${CommandName}"; }
  static QString postProcessMarker() { return "DoneDownloading"; }
...
public:
  ...
  const QStringList &otherOptions() const { return m_otherOptions; }
...
public:
  ...
  template <typename T> void removeOptionWithArgument(const T &opt) const {
    this->hasValue(opt, true);
  }
  template <typename T> void removeOption(const T &opt) const {
...
template <typename T> void removeArgument(QStringList &s, const T &e) {
  s.removeAll(e);
}
...
void openGetListOptionHelp();
void openDownloadOptionHelp();
...
class contextState {
public:
  contextState() : m_noneAreRunning(true), m_finishedSuccess(false) {}
  contextState(bool r) : m_noneAreRunning(r), m_finishedSuccess(false) {}
  contextState(bool r, bool f) : m_noneAreRunning(r), m_finishedSuccess(f) {}
  bool noneAreRunning() const { return m_noneAreRunning; }
  bool finishedSuccess() const { return m_finishedSuccess; }
  bool showLogWindow() const { return m_showLogWindow; }
  void setShowLogWindow() { m_showLogWindow = true; }
...
bool showHistory(QLineEdit &lineEdit, const QStringList &history,
                 Settings &settings, TabName tabName, Function function,
...
bool showHistory(QLineEdit &lineEdit, const QStringList &history,
                 Settings &settings, TabName tabName,
                 PlayListButtonName pbn = PlayListButtonName::None) {
  return utility::showHistory(
      lineEdit, history, settings, tabName, [](QMenu &) {}, pbn);
}

template <typename Function, typename AddAction>
void addDownloadContextMenu(bool running, bool finishSuccess, QMenu &m, int row,
                            Function function, AddAction addAction) {
  ...
  if (finishSuccess) {
    ...
    if (running) {
      ...
      ac = m.addAction(QObject::tr("Start Download"));
      ac->setEnabled(false);
    } else {
      if (function(row)) {
...
void appendContextMenu(QMenu &m, utility::contextState c,
                       const Function &function, bool showClear = true) {
  ...
  if (showClear) {
    ...
    auto ac = m.addAction(QObject::tr("Clear"));
...
public:
  ...
  QString text() const { return m_ac->text(); }
  QString objectName() const { return m_ac->objectName(); }

private:
  QAction *m_ac;
};

QMenu *setUpMenu(const Context &ctx, const QStringList &, bool addClear,
                 bool addOpenFolder, bool combineText, QWidget *parent);

template <typename Function>
void setMenuOptions(const Context &ctx, const QStringList &opts, bool addClear,
                    bool addOpenFolder, QPushButton *w, Function function) {
  auto m = w->menu();
...
void run(const QString &cmd, const QStringList &args, WhenDone w, WithData p) {
  util::run(
      cmd, args, [](QProcess &) {}, std::move(w), std::move(p));
}

template <typename Function, typename FunctionConnect> class Conn {
public:
  Conn(Function function, FunctionConnect functionConnect)
      : m_function(std::move(function)),
        m_functionConnect(std::move(functionConnect)) {}
  template <typename Fnt> void connect(Fnt function) {
  ...
  void disconnect() { QObject::disconnect(m_conn); }
...
utility::Conn<Function, FunctionConnect> make_conn(Function f,
                                                   FunctionConnect c) {
...
public:
  ...
  auto setUp(Object obj, Member member, int idx) {
    return utility::make_conn(
        [](const engines::engine &engine, QProcess &exe, int index, int idx) {
          return utility::Terminator::terminate(engine, exe, index, idx);
        },
        [idx, obj, member, this](auto function) {
          Q_UNUSED(
              this) // Older version of gcc seems to require capturing "this".

          return QObject::connect(
              obj, member,
              [idx, function = std::move(function)]() { function(idx); });
        });
  }
  auto setUp() {
    return utility::make_conn(
        ...
        [this](auto function) {
          ...
          return QObject::connect(
              this, m,
              [function = std::move(function)](int index) { function(index); });
        });
  }
...
private:
  ...
  static bool terminate(const engines::engine &, QProcess &exe, int index,
                        int idx) {
    if (index == idx) {
...
template <typename Function>
void setUpdefaultEngine(QComboBox &comboBox, const QString &defaultEngine,
                        Function function) {
...
public:
  ...
  void setVersion(const engines::engine &engine, Then then) {
    ...
    util::run(
        cmd.exe(), cmd.args(),
        [&engine, then = std::move(then)](const util::run_result &r) {
          if (r.success()) {
  ...
  networkAccess &network() { return m_networkAccess.get(); }
signals:
  void vinfoDone();
...
public:
  ProcessExitState(bool c, int s, int d, QProcess::ExitStatus e)
      : m_cancelled(c), m_exitCode(s), m_duration(d), m_exitStatus(e) {}
...
public:
  ...
  ProcessOutputChannels(QProcess::ProcessChannel c)
      : m_channelMode(QProcess::ProcessChannelMode::SeparateChannels),
        m_channel(c) {}
  QProcess::ProcessChannelMode channelMode() const { return m_channelMode; }
  QProcess::ProcessChannel channel() const { return m_channel; }
...
template <typename Tlogger, typename Options, typename Connection>
class context {
public:
  context(const engines::engine &engine, ProcessOutputChannels channels,
          Tlogger &&logger, Options &&options, Connection &&conn)
      : m_engine(engine), m_logger(std::move(logger)),
        m_options(std::move(options)), m_conn(std::move(conn)),
        m_channels(channels), m_timer(std::make_unique<QTimer>()),
        m_cancelled(false) {}
  void whenCreated(QProcess &exe, const engines::engine::exeArgs::cmd &cmd) {
    ...
    // disable outputting command into logger
    Q_UNUSED(cmd);
  ...
  void withData(QProcess::ProcessChannel channel, const QByteArray &data) {
    auto _withData = [&](const QByteArray &data) {
      ...
      if (!m_cancelled) {
        ...
        } else { // only log data when not list

          // qDebug() << "ADDING DATA TO LOG";

          if (lastLoggedLine != data) {
  ...
  void logError(const engines::engine::exeArgs::cmd &exe) {
    ...
    m_options.done(
        ProcessExitState(false, -1, -1, QProcess::ExitStatus::NormalExit));
  }
...
private:
  ...
  QByteArray m_data;
  QString lastLoggedLine;
  bool m_cancelled;
};

template <typename Connection, typename Tlogger, typename Options>
auto make_ctx(const engines::engine &engine, Options options, Tlogger logger,
              Connection conn, utility::ProcessOutputChannels channels) {
  using ctx = utility::context<Tlogger, Options, Connection>;

  return ctx(engine, channels, std::move(logger), std::move(options),
             std::move(conn));
}
...
void run(const QStringList &args, const QString &credentials, Ctx ctx) {
  ...
  if (cmd.valid()) {
    ...
    util::run(
        cmd.exe(), cmd.args(),
        [&cmd, ctx = std::move(ctx)](QProcess &exe) mutable {
        ...
        [](QProcess::ProcessChannel channel, const QByteArray &data,
           auto &ctx) { ctx.withData(channel, data); });
  } else {
    ctx.logError(cmd);
  }
}
...
public:
  typedef typename std::remove_reference_t<std::remove_cv_t<List>>::value_type
      value_type;
  typedef typename std::remove_reference_t<std::remove_cv_t<List>>::size_type
      size_type;

  reverseIterator(List s) : m_list(s), m_index(m_list.size() - 1) {}
  bool hasNext() const { return m_index > -1; }
  void reset() { m_index = m_list.size() - 1; }
  auto &next() {
  ...
  template <typename Function,
            util::types::has_bool_return_type<
                Function, typename reverseIterator<List>::value_type> = 0>
  ...
  template <typename Function,
            util::types::has_void_return_type<
                Function, typename reverseIterator<List>::value_type> = 0>
  void forEach(Function function) {
    while (this->hasNext()) {
...
template <typename List> auto make_reverseIterator(List &&l) {
  return reverseIterator<decltype(l)>(std::forward<List>(l));
}
class MediaEntry {
public:
  MediaEntry(const QString &url) : m_url(url), m_json(QByteArray()) {}
  MediaEntry(const QString &uiText, const QString &url)
      : m_title(uiText), m_url(url), m_json(QByteArray()) {}
  ...
  const QString &uploadDate() const { return m_uploadDate; }
  bool valid() const { return m_json; }
  const QJsonDocument &doc() const { return m_json.doc(); }
  QString errorString() const { return m_json.errorString(); }
  const QString &duration() const { return m_duration; }
  const QString &id() const { return m_id; }
  int intDuration() const { return m_intDuration; }
...
void updateFinishedState(const engines::engine &engine, settings &s,
                         tableWidget &table, const FinishedState &f) {
  ...
  table.setUiText(engine.updateTextOnCompleteDownlod(a, backUpUrl, ss, es),
                  index);
...
template <typename Opts, typename Functions> class options {
public:
  options(Opts opts, Functions functions)
      : m_opts(std::move(opts)), m_functions(std::move(functions)) {}
  void done(utility::ProcessExitState e) {
    m_functions.done(std::move(e), m_opts);
  }
  void listRequested(QByteArray e) { m_functions.list(std::move(e)); }
  bool listRequested() { return m_opts.listRequested; }
  int index() { return m_opts.index; }
  const QString &debug() { return m_opts.debug; }
  void disableAll() { m_functions.disableAll(m_opts); }
  QString downloadFolder() const { return utility::downloadFolder(m_opts.ctx); }
...
template <typename List, typename DisableAll, typename Done> struct Functions {
  List list;
...
Functions<List, DisableAll, Done>
OptionsFunctions(List list, DisableAll disableAll, Done done) {
